-- UNDETECTED GAMEPASSES UNLOCKER v1 – ALL BOOLS TRUE (Delta Safe) 😈🔥
local Players = game:GetService("Players")
local RunS = game:GetService("RunService")
local player = Players.LocalPlayer
local username = player.Name

-- Wait for Gamepasses folder
local gamepassesFolder = player:WaitForChild("Gamepasses", 10)
if not gamepassesFolder then
    error("Gamepasses folder not found – wrong game? 🤡")
end

local allUnlocked = true  -- Start all true
local selectedPass = nil  -- For manual toggle

print("Gamepasses folder locked | Found: " .. #gamepassesFolder:GetChildren() .. " passes")

-- AUTO UNLOCK ALL (Set all BoolValues to true)
local function unlockAll()
    for _, pass in pairs(gamepassesFolder:GetChildren()) do
        if pass:IsA("BoolValue") then
            pass.Value = true
            print("Unlocked: " .. pass.Name .. " 🔥")
        end
    end
    allUnlocked = true
end
...
sg.Name = math.random(1e9,9e9).."PassGui"
...
title.Text = "Gamepasses Unlocker – " .. username .. " 👑"
...
status.Text = "All Unlocked! | Passes: " .. #gamepassesFolder:GetChildren()
...
lockBtn.Position = UDim2.new(0, 160, 0, 90)
...
lockBtn.MouseButton1Click:Connect(function()
    ...
    allUnlocked = false
    status.Text = "All Locked | Passes: " .. #gamepassesFolder:GetChildren() .. " 💀"
end)
...
mt.__index = newcclosure(function(self, key)
    if self.Parent == gamepassesFolder and key == "Value" and allUnlocked then
...
RunS.Heartbeat:Connect(function()
    ...
    status.Text = "Tokens: " .. tokens.Value .. " | Status: " .. (allUnlocked and "Unlocked" or "Locked")
end)

print("GAMEPASSES UNLOCKER v1 LOADED – All BoolValues TRUE | Drag GUI & Rejoin Safe 😈")
